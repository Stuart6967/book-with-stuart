...
export default function AdminPage() {
  ...
  if (!authenticated) return <main className="shell success adminLogin"><div className="successIcon"><LockKeyhole size={30}/></div><p className="eyebrow">Administrator</p><h1>Beschikbaarheid beheren</h1><p className="lead">Alleen voor Stuart. Vul je administratorwachtwoord in.</p><form onSubmit={login} className="adminLoginForm"><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Administratorwachtwoord" required/><button className="secondary" type="submit" disabled={checking}>{checking?'Controleren...':'Inloggen'}</button></form>{error&&<div className="errorBox">{error}</div>}</main>;
